/**********************************************************************
 * bot.ts – VayaRide Driver Bot
 * --------------------------------------------------------------------
 * • Registration: name → phone → required docs
 * • Approval: 4‑digit PIN (24 h) → dashboard link
 * • Commands: /status  /newpin  /reset  /help
 *********************************************************************/
...
/* ------------------------------------------------------------------ */
/* 2 ▸ Finite‑state registration sessions                             */
/* ------------------------------------------------------------------ */
...
bot.on('message', async (m: Message) => {
  ...
  if (step === 'docs') {
    ...
    /* c) Decide Cloudinary resource type */
    ...
    else return bot.sendMessage(chat,'❌ Unsupported file – only JPG/PNG or PDF.')
    ...
    try {
      ...
      upload = await new Promise((res, rej) => {
        const s = cloud.uploader.upload_stream(
          { folder:`vayaride/${chat}`, public_id:nextKey, resource_type:resType, timeout:180_000 },
      ...
      return bot.sendMessage(chat,`❌ Upload failed – ${(err as any)?.message}`)
    }

    /* e) Save */
    await d.addOrUpdateDocument(nextKey, {
      fileId,
      fileUniqueId: m.document?.file_unique_id ?? m.photo![m.photo!.length-1].file_unique_id,
    ...
    await d.save()
    session.delete(chat)
    bot.sendMessage(chat,'🎉 Docs uploaded! We’ll review and notify you.', mainMenu(d))
  }
  ...
  if (step === 'set_pin' && m.text) {
    ...
    await d.save()
    session.delete(chat)
    return bot.sendMessage(chat,'✅ PIN saved. It will expire tomorrow 23:59.', mainMenu(d))
  }
})

/* ------------------------------------------------------------------ */
/* 8 ▸ Helper fired by Admin when driver approved                     */
/* ------------------------------------------------------------------ */
